...
function policyCleanup() {
  ...
  const ctlimage = manifest.focusedChecks.find(({ id }) => id === 'ctlimage');
  assert.deepEqual([ctlimage.activation, ctlimage.owner, ctlimage.argv], ['active', 'G015-activate-basic-ctlimage-control', ['node', 'scripts/verify-ui.mjs', '--phase', 'ctlimage']]);
  const controlModulesCheck = manifest.focusedChecks.find(({ id }) => id === 'control-modules');
  assert.deepEqual([controlModulesCheck.activation, controlModulesCheck.owner, controlModulesCheck.argv], ['active', 'G016-separate-control-modules', ['node', 'scripts/verify-ui.mjs', '--phase', 'control-modules']]);
  ...
  assert.deepEqual(manifest.stories.find(({ id }) => id.startsWith('G015-')), { id: 'G015-activate-basic-ctlimage-control', activation: 'active', checks: ['ctlimage'], budgetSeconds: 120 });
  assert.deepEqual(manifest.stories.find(({ id }) => id.startsWith('G016-')), { id: 'G016-separate-control-modules', activation: 'active', checks: ['control-modules'], budgetSeconds: 120 });
  ...
  const safeSources = ['src/xmf.ts', 'src/runtime-client.ts', 'src/XmfScreen.tsx', 'src/controls/ControlView.tsx', ...controlModuleFiles, 'src/controls/index.ts', 'src/controls/types.ts', 'App.tsx'];
