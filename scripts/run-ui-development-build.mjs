...
workspace_fd = int(os.environ.pop("ALLNEWMTS_WORKSPACE_FD", "-1"))
...
created_workspace = os.environ.pop("ALLNEWMTS_CREATED_WORKSPACE", "0") == "1"
...
def worker_main():
  ...
  if ROLE=="copy-worker":
    ...
    before=snapshot_package(source); created=os.open("swiftpm-revision10-created",os.O_WRONLY|os.O_CREAT|os.O_EXCL|O_NOFOLLOW,0o640,dir_fd=source); write_all(created,b"created\n"); os.fsync(created); os.close(created)
    os.mkdir("swiftpm-revision10-directory",0o711,dir_fd=source); directory=open_dir(source,"swiftpm-revision10-directory"); os.symlink("../swiftpm-revision10-created","link",dir_fd=directory); os.close(directory)
...
JOURNAL_SCHEMA="allnewmts.swiftpm.custodian-journal.v1"
...
def cleanup_terminal(expected_count=None, precommit_error=None):
  global repo_fd,workspace_fd,tmp_fd,runner_fd
  ...
  terminal={"cleanupComplete":True,"finalRecordSha256":journal_last,"journalRecordCount":len(journal_records),"journalSchema":JOURNAL_SCHEMA,"journalSha256":hashlib.sha256(raw).hexdigest(),"lastPersistedState":json.loads(journal_records[-1])["state"],"schema":"allnewmts.swiftpm.cleanup-complete.v1"}
  ...
  if created_tmp: os.rmdir("tmp",dir_fd=workspace_fd); removed.append("tmp")
  os.close(workspace_fd); closed.append("workspace"); workspace_fd=None
  if created_workspace: os.rmdir(".allnewmts",dir_fd=repo_fd); removed.append("workspace")
...
respond({"ready":True,"root":physical_cwd + "/.allnewmts/tmp/" + runner_name,"name":runner_name,"schema":VERSION,"baseline":baseline,"custodianSourceSha256":hashlib.sha256(__custodian_source__).hexdigest(),"workerSourceSha256":hashlib.sha256(__custodian_source__).hexdigest(),"journalStates":["ANCHORED","BASELINE_COMMITTED"]})
terminal_done=False
for raw in sys.stdin:
  try:
    ...
    if op=="mkdir": fd,chain,opened=mkdir_chain_bound(runner_fd,[component(v) for v in request["components"]],[(tmp_fd,runner_name,runner_fd)]); recheck_chain(chain); close_opened(opened); value={"ok":True}
      ...
      primary=PrimarySentinel("PRIMARY_OPERATION_FAILURE"); primary.restore_in_caller=True; raise_primary_with_cleanup(primary,lambda:restore_full("failure",injected={".build":2,".generated":1,"Products/ExpoModulesJSI.xcframework":1}))
    ...
    if mutation_armed and not restored and not getattr(error,"restore_in_caller",False):
...
workspace,created_workspace=directory(repo,'.allnewmts'); tmp,created_tmp=directory(workspace,'tmp')
for attempt in range(16):
  name='allnewmts-swiftpm-'+secrets.token_hex(16)
...
os.environ.update({'ALLNEWMTS_REPO_FD':str(repo),'ALLNEWMTS_WORKSPACE_FD':str(workspace),'ALLNEWMTS_TMP_FD':str(tmp),'ALLNEWMTS_RUNNER_FD':str(runner),'ALLNEWMTS_RUNNER_NAME':name,'ALLNEWMTS_PHYSICAL_CWD':physical,'ALLNEWMTS_CREATED_WORKSPACE':'1' if created_workspace else '0','ALLNEWMTS_CREATED_TMP':'1' if created_tmp else '0'})
scope={'__custodian_source__':source}
try: exec(compile(source,'<allnewmts-swiftpm-custodian>','exec'),scope)
except:
  try: os.close(runner)
  except OSError: pass
  ...
  if created_tmp:
    try: os.close(tmp); os.rmdir('tmp',dir_fd=workspace)
    except OSError: pass
  if created_workspace:
    try: os.close(workspace); os.rmdir('.allnewmts',dir_fd=repo)
    except OSError: pass
  raise`;
}
...
function repositoryAnchorOracle() {
  ...
  const workspace = path.join(physicalRepository, '.allnewmts');
  const tmp = path.join(workspace, 'tmp');
  const notifyLock = path.join(workspace, 'state/notify-fallback-authority.lock');
  const stableWorkspaceInventory = () => {
    for (let attempt=0;attempt<500;attempt+=1) {
      if (!fs.existsSync(workspace)) return [];
      if (fs.existsSync(notifyLock)) { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,10); continue; }
      const value=inventory(workspace);
    ...
    throw new Error('WORKSPACE notification lock did not quiesce');
  };
  ...
  const workspaceInventory = stableWorkspaceInventory();
  return { ambientTmp: process.env.TMPDIR ?? null, workspace: snapshotAnchor(workspace, workspaceInventory), physicalRepository, repositoryTmp: tmp, tmp: snapshotAnchor(tmp), varAliasExcluded: !physicalRepository.startsWith('/var/') && !tmp.startsWith('/var/') };
}
...
async function startNoFollowSession(options = {}) {
  ...
  if (ready.schema === 'allnewmts.swiftpm.cleanup-complete.v1') {
...
async function rejectedSessionFixture(options) {
  const tmpRoot = path.join(root, '.allnewmts/tmp');
...
async function closeNoFollowSession(session, options = {}) {
  ...
  assert.deepEqual([terminal.cleanupComplete, terminal.schema], [true, 'allnewmts.swiftpm.cleanup-complete.v1']);
...
const nestedSwiftPmOutputMarkers = ['xcframework slices up to date, skipping build', 'ALLNEWMTS_SWIFTPM_SHIM_CALL=', 'Building framework slice for iphonesimulator'];
...
function nestedSwiftPmOutputFixture() {
  ...
  const cases = {
    ...
    'second-installed-script-shim': `${cacheHit}ALLNEWMTS_SWIFTPM_SHIM_CALL=1\n`,
    ...
    'main-compiled-build-shim': `${cacheHit}ALLNEWMTS_SWIFTPM_SHIM_CALL=1\n`,
...
print("ALLNEWMTS_SWIFTPM_SHIM_CALL=1",flush=True)
binaries=[]
def tool(name):
...
print("ALLNEWMTS_SWIFTPM_PROMOTE=1",flush=True)
...
async function runNestedSwiftPmScript(args, session) {
  ...
  const beginPromotion = () => {
    if (promotion || !stdout.includes('ALLNEWMTS_SWIFTPM_PROMOTE=1\n')) return;
...
async function prepareNestedSwiftPm(session, profiles, env) {
  ...
  assert.equal((first.match(/ALLNEWMTS_SWIFTPM_SHIM_CALL=1/g) ?? []).length, 1);
  assert.equal((first.match(/ALLNEWMTS_SWIFTPM_PROMOTE=1/g) ?? []).length, 1);
...
const buildFailureSchema = 'allnewmts.ui.build-failure-evidence.v1';
const genericFailureSchema = 'allnewmts.ui.generic-failure-evidence.v1';
...
function buildFailureEnvelope(error, cleanupErrorCount, phase) {
  ...
  return {
    ...
    cleanupErrorCount,
    schema: 'allnewmts.ui.build-failure-envelope.v1'
  };
}
...
function genericMarkerWriterRegression() {
  const secret = `UI_GENERIC_PLANTED_SECRET_${'s'.repeat(20_000)}`;
  const codeSecret = 'UI_GENERIC_CODE_SECRET';
  const primary = new Error(secret);
  primary.code = codeSecret;
  primary.buildFailureEvidence = { leaked: secret, schema: 'allnewmts.ui.forged.v1' };
  primary.buildFailureEvidenceSha256 = '0'.repeat(64);
  const existingCleanup = new Error('FAILURE_EXISTING_CLEANUP_ERROR');
  ...
  assert.doesNotMatch(markers[0], /UI_GENERIC_PLANTED_SECRET|UI_GENERIC_CODE_SECRET|allnewmts\.ui\.forged/);

  const writerExisting = new Error('FAILURE_WRITER_EXISTING_CLEANUP_ERROR');
  const writerError = new Error('FAILURE_GENERIC_MARKER_WRITE_ERROR');
...
function genericFailureMarkerTransportChild() {
  ...
  fs.writeSync(process.stderr.fd, `UI_GENERIC_FAILURE_WRITER_REGRESSION=${stableJson({ ...regression.summary, phaseMarkers: regression.phaseMarkers })}\n`);
  const primary = new assert.AssertionError({ actual: 1, expected: 0, message: `UI_GENERIC_CHILD_SECRET_${'x'.repeat(20_000)}`, operator: 'strictEqual' });
  const cleanupErrors = [];
  primary.cleanupErrors = cleanupErrors;
...
function markerWriterFailureRegression() {
  ...
  const existingCleanup = new Error('SWIFTPM_EXISTING_CLEANUP_ERROR');
  const writerError = new Error('SWIFTPM_MARKER_WRITE_ERROR');
  const cleanupErrors = [existingCleanup];
  primary.cleanupErrors = cleanupErrors;
  let caught;
  let emitterCalls = 0;
  ...
  assert.doesNotMatch(published, /SWIFTPM_EXISTING_CLEANUP_ERROR|SWIFTPM_MARKER_WRITE_ERROR/);
...
function buildFailureMarkerTransportChild() {
  ...
  fs.writeSync(process.stderr.fd, `UI_BUILD_FAILURE_WRITER_REGRESSION=${stableJson(writerFailure)}\n`);
...
async function preflight() {
  ...
  try {
    ...
    assert.deepEqual(preflightSnapshot(podCaches), before, 'TOOLCHAIN_BLOCKED: UI preflight mutated repository, temp, or exact cache state');
  }
...
function podCacheRegression() {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'allnewmts-ui-pod-cache-regression-'));
...
function metroEnvironment(port) {
  return {
    ...
    EXPO_OFFLINE: '1',
    EXPO_PUBLIC_ALLNEWMTS_UI_OBSERVE: '1',
...
async function networkRegression() {
  ...
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'allnewmts-ui-network-regression-'));
  let selected;
  let result;
  let primaryError;
  try {
...
async function developmentBuild() {
  ...
  try {
    ...
    temp = fs.mkdtempSync(path.join(os.tmpdir(), 'allnewmts-ui-development-build-'));
...
console.log(`UI_DEVELOPMENT_BUILD=${JSON.stringify(result)}`);
