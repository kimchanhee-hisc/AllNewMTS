...
const requireConfiguration = (valid, message) => {
  if (valid) return;
  console.error(`FAIL MCI BETA S00 probe configuration: ${message}`);
  process.exit(64);
};
...
requireConfiguration(
  process.env.ALLNEWMTS_MCI_LIVE_BETA_REAL === 'S00:005930',
  'ALLNEWMTS_MCI_LIVE_BETA_REAL=S00:005930 is required');
...
const temporary = fs.mkdtempSync(
  path.join(os.tmpdir(), 'allnewmts-mci-realtime-probe-'));
...
try {
  ...
  const executable = path.join(temporary, 'mci-beta-realtime-probe');
