...
const requireConfiguration = (valid, message) => {
  if (valid) return;
  console.error(`FAIL MCI BETA probe configuration: ${message}`);
  process.exit(64);
};
...
requireConfiguration(process.env.ALLNEWMTS_MCI_LIVE_BETA === '1',
  'ALLNEWMTS_MCI_LIVE_BETA=1 is required');
...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'allnewmts-mci-probe-'));
...
try {
  ...
  process.stdout.write(run(executable, [source], { timeout: 25000 }));
} finally {
