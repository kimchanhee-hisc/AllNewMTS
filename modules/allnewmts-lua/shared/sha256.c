...
static void block(uint32_t state[8], const unsigned char input[64]) {
  uint32_t words[64], a, b, c, d, e, f, g, h;
  int i;
  for (i = 0; i < 16; i++) {
    words[i] = ((uint32_t)input[i * 4] << 24) | ((uint32_t)input[i * 4 + 1] << 16) |
...
void allnewmts_sha256(const unsigned char *data, size_t size, unsigned char out[32]) {
  ...
  size_t whole = size & ~(size_t)63, remaining = size - whole, padded, i;
  for (i = 0; i < whole; i += 64) block(state, data + i);
  memcpy(tail, data + whole, remaining);
  tail[remaining] = 0x80;
  padded = remaining < 56 ? 64 : 128;
  for (i = 0; i < 8; i++) tail[padded - 1 - i] = (unsigned char)(bits >> (i * 8));
  block(state, tail);
