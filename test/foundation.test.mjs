...
test('G004 runner and verifier keep hostile evidence and cleanup fail-closed', () => {
  ...
  assert.match(runner, /container\.status !== 0[\s\S]+legacyAbsence[\s\S]+container\.status === 1[\s\S]+xcode26Absence[\s\S]+container\.status === 2[\s\S]+NSPOSIXErrorDomain[\s\S]+legacyAbsence \|\| xcode26Absence[\s\S]+unexpected nonzero result/, 'only exact supported not-installed container results may count as absence');
