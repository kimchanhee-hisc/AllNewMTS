...
- `npm run verify:fast` runs format, documentation, policy, type, unit, and layer-boundary checks.
- `npm run verify:ci` runs every check listed in [`verification/manifest.json`](../verification/manifest.json).
- `npm run verify:layers`, `verify:fixtures`, `verify:networking`, `verify:native`, `verify:runtime`, `verify:ui`, `verify:ctlimage`, `verify:control-modules`, and `verify:provenance` run focused checks directly.
...
- `npm run app:allnewmts:ios` and `npm run app:allnewmts:android` launch the product with its packaged BETA-only MCI resource.
- `npm run lab:xmf -- ios|android` and `npm run lab:networking -- ios|android` launch the isolated module targets; the networking runner supplies only its bounded loopback server.
...
npm run mci:probe:beta -- --platform ios --source apps/allnewmts/assets/ip.dat
```
...
npm run mci:probe:beta:tr -- --platform ios --source apps/allnewmts/assets/ip.dat
```
...
npm run mci:probe:beta:realtime -- --platform ios --source apps/allnewmts/assets/ip.dat
```
