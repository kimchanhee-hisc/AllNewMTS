...
Networking transport semantics, credentials, dependency and remote boundaries, provenance, and deterministic scenarios are owned by [`networking-contract.md`](networking-contract.md). The current production runtime only stages `requestTranData` and performs no network transport.
