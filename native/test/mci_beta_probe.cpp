...
int main(int argc, char **argv) {
  const char *live = std::getenv("ALLNEWMTS_MCI_LIVE_BETA");
  if (argc != 2 || !live || std::strcmp(live, "1") != 0) {
    std::cerr << "FAIL MCI BETA probe configuration\n";
    return 64;
  }
  ...
  if (size <= 0 || size > 64 * 1024) {
    std::cerr << "FAIL MCI BETA probe source\n";
    return 65;
  }
  source.seekg(0);
  ...
  if (code == ALLNEWMTS_MCI_OK)
    code = allnewmts_mci_probe_beta(client, bytes.data(), bytes.size());
  allnewmts_mci_destroy(client);
  allnewmts_mci_socket_destroy(transport_context);

  if (code != ALLNEWMTS_MCI_OK) {
    std::cerr << "FAIL MCI BETA init probe code=" << code << '\n';
    return 1;
  }
  std::cout << "PASS MCI BETA init probe; socket closed; values redacted\n";
}
